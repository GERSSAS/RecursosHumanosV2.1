...
if($gatillo==0){
   ...
   if(isset($_FILES['usu_cartapresentacion'])){
    ...
    $sql="INSERT INTO tbl_usuariovacante(ofer_id,usu_id,id_vacante,id_seleccionado,usu_viajar,usu_elegible,usu_cartapresentacion,usuvac_fecha,usu_cancelado)
          VALUES($id,'".$_SESSION['idUser']."','".$id_vacante."',3,'".$usu_viajar."','".$usu_elegible."','".$usu_cartapresentacion."',CURRENT_TIMESTAMP,NULL)";
...
$mail->ClearAddresses();
 ...
 //Recipients
 $mail->setFrom('no.reply@gers.com.co', 'Soporte Gers');
 $mail->addAddress('alba.naranjo@gers.com.co');     // Add a recipient
 //$mail->addAddress(".$Usu_email.");                    // Name is optional
 // $mail->addReplyTo('info@example.com', 'Information');
// $mail->addBCC('carolcundar19@gmail.com');
 // $mail->addCC('bcc@example.com');
...
<!-- aqui voy a imprimir lo de timeline -->
<div style="padding-top:10px;">
    <h5 class="card-title"><?php echo $idi['idi_nombre']?></h5>
    <p class="card-text"><b>Nivel de idioma: </b><?php echo $idi["idi_nivel"]?></p>  
    <br>
</div>
<div class="card-footer text-muted time">
</div>
